...
const Quickhub = () => (
  <Layout>
    ...
    <article>
      ...
      <h2>The Idea</h2>
      <p>
        I usually have a GitHub tab or two open and often find like many other apps they get
        buried in an overloaded tab bar.
      </p>
      ...
      <p>
        ...
        an auth variable to <InlineCode>true</InlineCode>.
        ...
        <i> "all good bro, I've got a <InlineCode>true</InlineCode> over here!" </i>
        🤦🏻‍♂️
      </p>
      <p>
      ...
      <p>
        ...
        <InlineCode> background.js</InlineCode> was getting big. It contained all the Firebase and GitHub
      ...
      <p>
        ...
        via <InlineCode>chrome.runtime.sendMessage </InlineCode> and
        <InlineCode> chrome.runtime.onMessage.addListener</InlineCode> methods
      </p>
      <p>
        I decided to build out a flow that implemented an "optimistic UI" of sorts
        using React Context and caching with Chrome storage.
      </p>
      <ol className="list-decimal list-inside">
        ...
        <li className="text-xl">
          Background listens for the message and returns local storage
          data and calls the GitHub API.
        </li>
        <li className="text-xl">
          Front end updates context state with existing local storage data.
        </li>
        <li className="text-xl">
          Fetch response is saved to <InlineCode>chrome.runtime.local.storage</InlineCode>.
        </li>
        <li className="text-xl">
          Background listens for changes to local storage
          and sends out a message with updates.
        </li>
        <li className="text-xl">
          Front end listens for storage changes and updates the
          context state
        </li>
      </ol>
      <figure className="my-8">
      ...
      <p>
        ...
        messages are sent in succession I ran into a race condition where
        the React state would get out of sync. This was fixed by combining the data into a
        single message on the background but it was still a gotcha to work around.
      </p>

      <hr className="my-8 opacity-25" />
      <h2>Starting to rethink things</h2>
      <p>
        By now I'm starting to get a better grasp of the chrome extension framework
        but because I was using create-react-app I had no control over the build
        so <InlineCode>background.js</InlineCode> was left hanging around outside
      ...
      <p>
        ...
        a <InlineCode>manifest.json</InlineCode> that I could now automate in the build process.
      </p>
      <p>
        Being able to add in a Tailwind.css build set was a bonus too.
      </p>
      <p>
        I also started to move only really "global" data into context to share between multiple
        components and component level data into local state.
        This does simplify the data flow and keeps state close to where it's used but it means
        I no longer have the optimistic UI.
      </p>
      ...
      <h2>Next</h2>
      <p>
        I would like to improve and simplify the data flow and bring back
        data caching for that fast UI.
      </p>
      <p>
        I never got to build with GraphQL, I went with REST because I'm more
        familiar with it which meant I could get moving quicker in the limited time
        I had. The REST API returns tons of info and I'm using almost none of it
        so moving to GraphQL makes a lot of sense.
      </p>
      <p>
        This app does work in Firefox as well but I discovered that Firebase doesn't
        support Firefox which is a bummer.
      </p>
      ...
      <p>
        ...
        aside from getting pretty conformable with the Chrome extension framework
      ...
      <p>
        ...
        YouTube, Medium, StackOverflow etc all offer loads of helpful insights
        but they're unlikely to cover your exact use case and "best practices"
        are a moving target.
        <br />
        That YouTube video I watched promised to walk me through the Firebase
        integration with a platform I was unfamiliar with. It took an approach
        I wasn't considering initially but I figured this would just get it done quickly.
        While I did learn some stuff I also had to undo the entire thing
        and go back to doing it the way I had imagined in the first place.
      </p>
      ...
      <Img width={1024} height={640} src="/quickhub/chrome-store.jpg" alt="quickhub" />
    </article>
  </Layout>
);
